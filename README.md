...
- COMMANDモードとNORMALモードで無名レジスタを共有し、`:y3` → `p/P` を利用可能
...
- バイナリモードでも `j/k`, `Ctrl+F/B`, `Ctrl+D/U`, `gg/G`, `/ ? n N` のvi操作を利用可能

## レジスタ共有とyank / paste

v0.1.11では、COMMANDモードの `:yank` とNORMALモードの `yy` / delete / pasteが同じ `ViRegisterStore` を使います。
...
これで、カーソル行から3行をyankした後、NORMALモードの `p` でその3行を貼り付けられます。`P` なら現在行の前へ貼り付けます。

逆方向も共有されるため、NORMALモードで `yy` した行を `:put` から利用できます。

なお、**参照モード中は安全のため `p/P` は従来どおり無効**です。貼り付ける場合は `モード` → `参照モード` のチェックを外して編集モードにしてください。yank自体は参照モード中でも利用できます。
...
`/` または `?` を押すと、通常のvi検索と同じように画面下部へ検索文字列を入力できます。

- `/文字列` : 前方検索
- `?文字列` : 後方検索
- `n` : 同じ方向へ繰り返し
- `N` : 逆方向へ繰り返し
- `/hex:4D 5A 90` : RAWバイト列 `4D 5A 90` を前方検索

通常文字列の検索は、バイナリ画面で現在選択している文字コード（UTF-8 / Shift_JIS / UTF-16 / ASCII）で検索文字をバイト列へ変換して実行します。

検索は64KB単位で `RandomAccess.Read` し、ファイル全体をメモリへ展開しません。検索結果が見つかると、そのバイトを含む16バイト行へ移動します。

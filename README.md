...
`表示` → `バイナリモード` で現在の保存済みファイルを生バイト列として閲覧できます。さらに **`ファイル` → `バイナリとして開く...`** を使うと、通常のテキストデコードを一切経由せず、任意のファイルをRAWバイトとして直接開けます。既存のテキスト編集バッファは保持され、バイナリモードを閉じると元のテキスト画面へ戻ります。

バイナリモードは読み取り専用です。画面は1行16バイトで次の3列を表示します。

...
- 既に起動中の場合は二重起動せず、渡されたファイルを既存ウィンドウの新規タブで開く
- Explorerからウィンドウへファイルをドラッグ＆ドロップして新規タブで開く
- テキスト内の `http://` / `https://` URLを入力・貼り付け・削除に追従して青色・下線付きリンク表示し、シングルクリックで既定ブラウザから開く
...
- Markdown lexerのリンク全体スタイルを通常文字色へ戻し、`[UiBank](https://example.com)（説明）`ではURL部分だけを青色・下線付きで表示
- Markdownリンクの表示名、角括弧、丸括弧、URL後の日本語をURL強調表示から除外
- 起動直後の未編集・空の`無題`タブが1つだけある場合、実ファイルを開いた後に自動削除
- 編集済みまたは文字入力済みの`無題`タブは自動削除せず保護
- タブ見出しをマウスでドラッグ＆ドロップして並べ替える操作を追加
- Windows実ScintillaでMarkdown URL範囲を検査し、無題タブ置換とタブ並べ替えをPortable／Single-file版で検査するスモークテストを追加

### v0.1.30

- NORMALモードで `d4d` のような `d{行数}d` を追加し、指定行数を削除すると同時に無名レジスタへ行単位で保存
- `d{行数}d` で削除した複数行を `p` / `P` で貼り付け可能にし、`.` による同じ行数の削除繰り返しにも対応
- change系を `cw` / `cW` / `ce` / `cE` / `c$` / `C` / `cc` へ拡張し、対象削除後にINSERTモードへ移行
- `cw` で単語全体を削除して入力文字へ置き換える実利用シナリオと、行数付き削除・ヤンク・貼り付けのCore回帰テストを追加

### v0.1.29

- ScintillaNETの文字位置とScintillaネイティブAPIのUTF-8バイト位置を混在させていた不具合を修正
- 日本語を含む複数行文書でも、URLとは別の行、Markdownリンクの表示名、行頭の空白へ青色・下線がずれないよう改善
- URLクリック判定もネイティブ位置からUTF-16文字位置へ正しく変換し、日本語の後にあるURLを確実に開くよう修正
- 実際のScintillaコントロール上で、日本語・Markdown・複数URLの全バイトに対する装飾範囲を検査するWindowsスモークテストを追加

### v0.1.28

- URL装飾範囲をScintilla自身のUTF-16位置変換で計算し、日本語やMarkdown書式が混在しても `http://` / `https://` の先頭から末尾だけを正確に装飾
- Markdownの箇条書きと `[表示名](https://example.com)` 形式で、行頭の空白、リスト記号、表示名、末尾の閉じ括弧をURL装飾へ含めないよう修正
- Markdownリンクの表示名にもURLが書かれている場合、表示名側とリンク先側を別々のURLとして検出
- 文書全体の装飾解除にもScintillaのネイティブ文書長を使用し、編集後の古い下線が残らないよう改善
- Markdown箇条書き、生URL、リンク表記、複数URLの厳密な検出範囲をCore単体テストへ追加

### v0.1.27

- テキスト内の `http://` / `https://` URLを、HTMLリンクのような青色・下線付き表示へ変更
- ファイル読込に加え、入力・貼り付け・削除の直後にURL表示を動的更新
- 装飾にはScintillaの表示用インジケーターを使用し、文書内容、保存データ、変更状態、Undo履歴を変更しない構成を採用
- 編集時は変更行と隣接行だけを再判定し、キー入力ごとの文書全体走査を回避
- 日本語を含む行でもUTF-8バイト位置へ変換し、URL範囲だけを正確に装飾
- 従来のURLシングルクリックによる既定ブラウザ起動と、URL上の手形ポインターを維持

### v0.1.26

- 開いたテキスト内の `http://` / `https://` URLを左シングルクリックすると、Windowsの既定ブラウザで開く機能を追加
- URL上ではマウスポインターを手の形へ変更し、クリック可能であることを表示
- URL以外のクリックによるキャレット移動と、マウスドラッグによる範囲選択は従来どおり維持
- 日本語がURLより前にある行でも、ScintillaのUTF-8位置と.NET文字位置の差を補正して正しいURLを判定
- URL直後の句読点や閉じ括弧を除外し、URL内部で対応している括弧は保持
- URL検出をCoreサービスとして分離し、日本語混在、句読点、括弧、HTTPS大文字表記、URL外クリックの単体テストを追加

### v0.1.25

- 名前付きMutexでvi_text_editorの通常起動をユーザーセッション内の1インスタンスに統一
- 起動中にExplorerのダブルクリック／「プログラムから開く」で渡されたファイルを、名前付きPipe経由で既存ウィンドウへ転送
- 既存ウィンドウは受信したファイルを新規タブで開き、最小化中の場合は通常表示へ復帰
- 複数ファイル、スペースを含むパス、既に開いているファイルの重複タブ抑止に対応
- Explorerからvi_text_editorへファイルをドラッグ＆ドロップし、新規タブとして開く操作に対応
- Pipeを現在のWindowsユーザーだけに制限し、無効なパス／不正メッセージで既存プロセスが終了しないよう防御
- CIへ実プロセス2個を使った既存インスタンス受信スモークテストを追加
...
以後は `.txt` のダブルクリックで対象ファイルがvi_text_editorに渡され、そのファイルを開きます。スペースを含むパスにも対応します。複数ファイルを一度に渡された場合は複数タブで開きます。v0.1.25以降、vi_text_editorが既に起動中なら別ウィンドウは作らず、既存ウィンドウの新規タブで開きます。

### 起動中のウィンドウへドラッグ＆ドロップする

Explorerでファイルを選び、起動中のvi_text_editorのタブまたは編集領域へドラッグ＆ドロップすると、新規タブで開きます。複数ファイルの同時ドロップにも対応し、既に同じファイルが開かれている場合は既存タブを選択します。
